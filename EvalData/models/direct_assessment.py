...
class DirectAssessmentTask(BaseMetadata):
    ...
    def next_item_for_user(self, user, return_completed_items=False):
        ...
        if not next_item:
            ...
            required_user_results = 1 #changed from 100
            if trusted_user:
                required_user_results = 1 #changed from 70
    ...
    def import_from_json(cls, campaign, batch_user, batch_data, max_count):
        ...
        for batch_task in batch_json:
            ...
            if not len(new_items) >= 1: #changed from 100
                _msg = 'Expected 10 items for task but found {0}'.format(
                    len(new_items)
                )
                LOGGER.warn(_msg)
                continue
...
class DirectAssessmentResult(BaseMetadata):
    ...
    def get_hit_status_for_user(cls, user):
        ...
        completed_hits = len([x for x in user_data.values() if x >= 5]) #changed from 100
    ...
    def compute_accurate_group_status(cls):
        ...
        for group_name in group_status:
            ...
            for task_id in task_ids:
                if group_status[group_name].count(task_id) >= 7: #chnaged from 70
                    completed_tasks += 1
